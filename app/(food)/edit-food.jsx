...
const EditFood = () => {
  ...
  useEffect(() => {
    // Function to fetch food details by ID
    const fetchFoodDetails = async () => {
      try {
        const response = await fetchData.getFoodById(food_id) // Adjust this to match your API call
        if (response) {
          setFoodDetails(response) // Assuming response structure matches foodDetails state
          // Set state values from response
          setName(response.name)
          setDescription(response.description)
          setCalories(String(response.calories_per_serving))
          setProtein(String(response.protein))
          setFat(String(response.fat))
          setCarbohydrate(String(response.carbohydrate))
          setType(response.type)
          setImage(response.images[0].image_url)
          setImageURL(response.images[0].image_url)
        } else {
  ...
  const editFood = async () => {
    try {
      ...
      const response = await fetchData.updateFood(
        token,
        {
          name,
          description,
      ...
      if (response) {
        Alert.alert('Success', 'Food edited successfully')
        router.back()
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView className='w-full h-full flex justify-center items-center'>
      <Text className='text-3xl font-pregular mt-4 mb-4'>Edit Food</Text>
      <ScrollView className='w-full px-4'>
        ...
        />
        {descriptionError ? (
          <Text className='text-red-500'>{descriptionError}</Text>
        ) : null}
        ...
        <View className='flex-row justify-between mt-2'>
          <View className='w-[48%]'>
            ...
            />
            {caloriesError ? (
              <Text className='text-red-500'>{caloriesError}</Text>
            ) : null}
          </View>
          <View className='w-[48%]'>
            <FormField
            ...
            />
            {proteinError ? (
              <Text className='text-red-500'>{proteinError}</Text>
            ) : null}
          </View>
        </View>
        <View className='flex-row justify-between mt-2'>
          ...
          <View className='w-[48%]'>
            ...
            />
            {carbohydrateError ? (
              <Text className='text-red-500'>{carbohydrateError}</Text>
            ) : null}
          </View>
        </View>
        <FormField
        ...
        <View className='mt-6 mb-4'>
          ...
          {imageError ? (
            <Text className='text-red-500'>{imageError}</Text>
          ) : null}
        </View>
